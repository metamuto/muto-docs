...
OHM has an intrinsic value of 1 DAI, which is roughly equivalent to $1. In order to make a profit from bonding, Muto charges a premium for each bond.

$$
Premium = debt Ratio * BCV
$$

The premium is derived from the debt ratio of the system and a scaling variable called [BCV](https://docs.Mutodao.finance/references/glossary#bcv). BCV allows us to control the rate at which bond prices increase.
...
At the end of each epoch, the treasury mints OHM at a set [reward rate](https://docs.Mutodao.finance/references/glossary#reward-rate). These OHM will be distributed to all the stakers in the protocol. You can track the latest reward rate on the [Muto Policy dashboard](https://dune.xyz/shadow/Muto-Policy).
...
The individual would supply 1 pOHM along with 1 DAI to mint 1 OHM. The pOHM is subsequently burned. Read [this Medium article](https://Mutodao.medium.com/what-is-poh-16b2c38a6cd6) for more information on pOHM.
...
Every OHM in circulation is backed by the Muto treasury. The assets in the treasury can be divided into two categories: stablecoin and non-stablecoin.

$$
treasuryBalance_{stablecoin} = RFV_{reserveBond} + RFV_{lpBond}
$$

The stablecoin balance in the treasury grows when bonds are sold. [RFV](https://docs.Mutodao.finance/references/glossary#rfv) is calculated differently for different bond types.
